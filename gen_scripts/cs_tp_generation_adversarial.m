function [n_tp,n_cs,cs_positions,tp_positions, cs_cs_distance_matrix, cs_tp_distance_matrix] = cs_tp_generation_adversarial(scenario)
%CS_TP_GENERATION_ADVERSARLIAL Summary of this function goes here
...
PLOT_SITE = 0;
%%
% cs positions are generated only once
...
if has_buildings
    partial_tp_positions = position_building_filter(partial_tp_positions,build_descr);
end
...
%% generating here starting positioning for tp

tp_positions = zeros(n_tp, 2, scenario.starting_tp_positioning);
cs_tp_distance_matrix = zeros(n_cs, n_tp, scenario.starting_tp_positioning);

for p=1:scenario.starting_tp_positioning
    ...
    partial_tp_positions = [xpos ypos];
    assert((size(cs_positions,1) >= n_cs & size(partial_tp_positions,1) >= n_tp), 'Number of CS or TP higher than maximum. In generateInstance')
    ...
    partial_tp_positions = partial_tp_positions(1:n_tp,:);
    ...
    if PLOT_SITE
        ...
        scatter(partial_tp_positions(:,1),partial_tp_positions(:,2),50,'d','filled');
    ...
    tp_positions(:,:,p) = partial_tp_positions;
    cs_tp_distance_matrix(:,:,p) = pdist2(cs_positions, partial_tp_positions);
