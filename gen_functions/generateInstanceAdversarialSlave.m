function [instance] = generateInstanceAdversarialSlave(scenario, instance_folder, dataname, rng_seed,solution, x_history)
...
%angles
% angles_mask_ris = zeros(n_tp,active_donor_cs,active_ris_cs);
...
% linlen_mask = bs_tp_distance_matrix' <= 80;
% linlen_mask = repmat(linlen_mask,1,1,active_ris_cs);
...
% prepare mask for angles: for all tps check if the angle is inside ris fov
temp_mask = false(n_tp,active_ris_cs);
for t=1:n_tp
    for r=1:active_ris_cs
        ...
        temp_mask(t,r) = temp_angle >= solution.delta(ris_cs_map(r)) + max_angle_span...
            || temp_angle <= solution.delta(ris_cs_map(r)) - max_angle_span;
    end
end

ris_p_mask = ris_p_mask & permute(repmat(temp_mask,1,1,active_donor_cs),[1,3,2]);
...
%     for d=1:active_donor_cs
%         for t=1:n_tp
%             ris_p_mask(t,d,r) = ris_p_mask & max_airtime(t,donor_cs_map(d),ris_cs_map(r)) < 1;
%         end
%     end
% end

% integrate x_history into the mask

%ris_p_mask = ris_p_mask & x_history(:,donor_cs_map, ris_cs_map);
