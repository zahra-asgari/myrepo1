...
function [Panel] = computeElemPos(Param,Orientation_rad,WhatToCalculate)
...
switch Param.Device
    case 'BS'
        if isequal(WhatToCalculate,'OnlyRotation')
            ...
            N_h = Param.Nh;
            N_v = Param.Nv;
            d = Param.d;   % inter-element sapcing
            a = (N_h-1) * d;
            shift = a * sqrt(3) / 6;
            %     shift = 0;
            ...
            Panel.ElemPos.Panel1 = Rot_Pan1_AlongZ * elPos_Tilted + Param.Center.';
            ...
            Panel.ElemPos.Panel2 = Rot_Pan2_AlongZ * elPos_Tilted + Param.Center.';
            ...
            Panel.ElemPos.Panel3 = Rot_Pan3_AlongZ * elPos_Tilted + Param.Center.';
        end
    ...
    case 'UE'
        ...
        Panel.ElemPos = Rot_Pan1_AlongZ * elPos_Tilted + Param.Center.';
        Panel.Orientation = UE_Rot;
        
    case 'RIS'
        RIS_Elem_locations = zeros(Param.Nris,1,1,3);
        Center = Param.Center;
        switch Param.Plane
            case 'xy'
                M = Param.Nh;
                N = Param.Nv;
                X_locations = Center(1) + (-(M - 1)/2 : 1 : (M - 1)/2) .* Param.d;
                Y_locations = Center(2) + (-(N - 1)/2 : 1 : (N - 1)/2) .* Param.d;
            ...
            case 'yz'
                M = Param.Nh;
                N = Param.Nv;
                Y_locations = Center(2) + (-(M - 1)/2 : 1 : (M - 1)/2) .* Param.d;
                Z_locations = Center(3) + (-(N - 1)/2 : 1 : (N - 1)/2) .* Param.d;
            ...
            case'zx'
                M = Param.Nh;
                N = Param.Nv;
                Z_locations = Center(3) + (-(M - 1)/2 : 1 : (M - 1)/2) .* Param.d;
                X_locations = Center(1) + (-(N - 1)/2 : 1 : (N - 1)/2) .* Param.d;
