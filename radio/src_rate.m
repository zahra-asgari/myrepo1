function [expected_rate_wris] = src_rate(d_direct,d_inc,d_ref, phi_inc, phi_ref)
%SCR_RATE Compute the outage-weighted rate for a src given their relative
...
%zero my the other 2 masks
states_pathloss = ...
    los_mask.*...
    repmat([pathloss(d_direct, 'los', 'linear') pathloss(d_inc, 'los', 'linear').*(cosd(phi_inc)^c_exp) pathloss(d_ref, 'los', 'linear').*(cosd(phi_ref)^c_exp)],size(states,1), 1)+...
    nlos_mask.*...
    repmat([pathloss(d_direct, 'nlos', 'linear') pathloss(d_inc, 'nlos', 'linear').*(cosd(phi_inc)^c_exp) pathloss(d_ref, 'nlos', 'linear').*(cosd(phi_ref)^c_exp)],size(states,1), 1);
...
expected_rate_wris = sum(states_prob.*states_rate);
%
