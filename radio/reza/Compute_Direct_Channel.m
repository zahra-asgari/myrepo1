...
Tx.Panel =  computeElemPos(Tx,Genaral_Tx_Orientation_for_Rx,'d');

Genaral_Rx_Orientation_for_Tx = Get_Orientation(Rx,Tx,Angles);
Rx.Panel =  computeElemPos(Rx,Genaral_Rx_Orientation_for_Tx,'d');
...
if DoesServe
    ...
    Normalized_Dir_Tx = Dir_Patch(wrapToPi(Angles.Tx.Rx - Tx_Orientation_selected) , wrapToPi(Theta_v_Tx2Rx -  Tx.DownTiltRad),Params.Tx);
    ...
    Normalized_Dir_Rx = Dir_Patch(wrapToPi(Angles.Rx.Tx - Rx_Orientation_selected) , wrapToPi(Theta_v_Rx2Tx -  Rx.DownTiltRad),Params.Rx);
