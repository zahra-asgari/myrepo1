...
AF_Horizontal_Limit = AF.Horizontal_FOV/2 + AF.Horizontal_Alignment_Limit;
...
General_Tx_Orientation_for_AF = Get_Orientation(Tx,AF,Angles);
Tx.Panel =  computeElemPos(Tx,General_Tx_Orientation_for_AF,'OnlyRotation');
...
General_Rx_Orientation_for_AF = Get_Orientation(Rx,AF,Angles);
Rx.Panel =  computeElemPos(Rx,General_Rx_Orientation_for_AF,'OnlyRotation');
...
if (~DoesServe1)  || (~DoesServe2) || (~DoesServe3)
    ...
    Normalized_Dir_Tx = Dir_Patch(wrapToPi(Angles.Tx.Relay- Tx_Orientation_selected) , wrapToPi(Theta_v_Tx2Relay - Tx.DownTiltRad),Params.Tx);
    Normalized_Dir_Rx = Dir_Patch(wrapToPi(Angles.Rx.Relay- Rx_Orientation_selected) , wrapToPi(Theta_v_Rx2Relay - Rx.DownTiltRad),Params.Rx);
    %     Normalized_Dir_Rx = 0;
    
    Normalized_Dir_Pan1 = Dir_Patch(wrapToPi(Angles.Relay.Tx - Pan1_Orient) , wrapToPi(Theta_v_Relay2Tx_Pan1 - Tilt_Relay_Pan1),Params.AF);
    Normalized_Dir_Pan2 = Dir_Patch(wrapToPi(Angles.Relay.Rx - Pan2_Orient) , wrapToPi(Theta_v_Relay2Rx_Pan2 - Tilt_Relay_Pan2),Params.AF);
