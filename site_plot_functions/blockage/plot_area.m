...
for di=51:51
    ...
    %for sol=14:14
        ...
        switch model_name
            %case {'iab_ris_fixedDonor_fakeRis_blockageModel_sumMCS','iab_ris_fixedDonor_fakeRis_blockageModel_sumrate'}
            case {'iab_ris_fixedDonor_fakeRis_blockageModel_sumrate','iab_ris_fixedDonor_fakeRis_blockageModel_sumextra','iab_ris_fixedDonor_fakeRis_blockageModel_sumrateforextra'}
...
%                 if ismember(str2double(data_name),[52,57,59,66,74,99])
                ...
                site_cache_path = ['cache/containment_for_tests/' site_cache_id '.mat'];
                radio_cache_path = ['cache/containment_for_tests/' radio_cache_id '.mat'];
                if isfile(site_cache_path)
                    load(site_cache_path,...
                        'cs_positions','tp_positions','scenario','n_cs','n_tp','cs_tp_angles','smallest_angles');
                else
                ...
                if isfile(radio_cache_path)
                    load(radio_cache_path,...
                        'weighted_rates','ris_rates','state_probs','ris_better_mask');
                else
                   disp("Site cache not found!!")
                   continue;
                ...
                xlabel('UTM Grid Zone 32T X Coordinates [m]');
                ylabel('UTM Grid Zone 32T Y Coordinates [m]');
                axis equal;
%                 title(['Hexagonal Cell, Instance ' data_name ', Budget ' num2str(scenario_struct.sim.budget(di)) ', ' model_title ' model']);
                ...
                %sites
                used_iab = (sum(f)>1)';
                used_ris = squeeze(sum(x.*ris_rates,[1,2]) >1);
%                 if or(sum(used_ris)==0,sum(used_ris)>2)
%                     continue;
%                 end
%                 if sum(used_ris)>0
%                     continue;
...
%                 end
                ...
                if THREED
                    ...
                    scatter3(cs_positions(used_iab == 1,1),cs_positions(used_iab == 1,2),repmat(scenario_struct.radio.iab_height,[size(cs_positions(used_iab == 1),1),1]),marker_size,'^b','filled');
                    plot3([cs_positions(used_iab == 1,1) cs_positions(used_iab == 1,1)],[cs_positions(used_iab == 1,2) cs_positions(used_iab == 1,2)],[0 scenario_struct.radio.iab_height], 'Color','k','HandleVisibility','off');
                    scatter3(cs_positions(used_ris == 1,1),cs_positions(used_ris == 1,2),repmat(scenario_struct.radio.ris_height,[size(cs_positions(used_ris == 1),1),1]),marker_size,'sm','filled');
%                     plot3([cs_positions(used_ris == 1,1) cs_positions(used_ris == 1,1)],[cs_positions(used_ris == 1,2) cs_positions(used_ris == 1,2)],[0 scenario_struct.radio.ris_height], 'Color','k','HandleVisibility','off');
...
%                     view([0 45]);
                    ...
                    scatter(cs_positions(used_iab == 1,1),cs_positions(used_iab == 1,2),marker_size,'^b', 'filled');
                    scatter(cs_positions(used_ris == 1,1),cs_positions(used_ris == 1,2),marker_size,'sm', 'filled');
                end
                ...
                legend_cells={'IAB Donor', 'IAB Node', 'RIS', 'Test Point'};
...
%                         pv = [tempBuildings(b).geometry.coordinates; tempBuildings(b).geometry.coordinates(1,:)];
                ...
                l = legend(legend_cells,'Position',[0.735460073984641,0.630704372175155,0.218749995459803,0.249255945285162]);
...
%                 end
    ...
    sdf(export_style);
    if SAVE_PICS
        saveas(gcf,['site_plot_functions/blockage/pictures_extra_100/' num2str(scenario_struct.sim.budget(di)) '.png']);
    end
end
