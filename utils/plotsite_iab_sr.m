...
scatter(cs_positions(y_rel(1:end-1) == 1,1),cs_positions(y_rel(1:end-1) == 1,2),100,'sm', 'filled');

legend('Construction Sites', 'Test Points', 'Donors', 'IAB nodes', 'SR');
...
for c=1:n_cs
    for d=1:n_cs-1
        if x(c,d) == 1
            if first
                plot([cs_positions(c,1) cs_positions(d,1)],[cs_positions(c,2) cs_positions(d,2)],'k--', 'DisplayName', 'Relay Link');
                first = 0;
            else
                plot([cs_positions(c,1) cs_positions(d,1)],[cs_positions(c,2) cs_positions(d,2)], 'k--','HandleVisibility','off');
            end
        ...
        for r=1:n_cs-1
            if s(t,c,r) == 1
                if first
                    ...
                    plot([cs_positions(r,1) tp_positions(t,1)],[cs_positions(r,2) tp_positions(t,2)], 'k-.', 'DisplayName', 'Access Link');
