...
if PLOT_CLUSTERS
    ...
    ps ( all(~ps, 2), : ) = [];
    %[IDX,C,SUMD,K,D]=best_kmeans(ps);
    [IDX,C,SUMD,D] = kmeans(ps,3);
    plot(C(:,1),C(:,2),'ko',...
        'MarkerSize',15,'LineWidth',3)
    legend('Construction Sites', 'Test Points', 'Donors', 'IAB nodes','Cluster Centroids');
else
    legend('Construction Sites', 'Test Points', 'Donors', 'IAB nodes');
end
sdf('Polimi-ppt');
%% backhaul links
first = 1;
for c=1:n_cs
    for d=1:n_cs
        if z(c,d) == 1
            if first
                plot([cs_positions(c,1) cs_positions(d,1)],[cs_positions(c,2) cs_positions(d,2)],'k', 'DisplayName', 'BH Link');
                first = 0;
            else
                plot([cs_positions(c,1) cs_positions(d,1)],[cs_positions(c,2) cs_positions(d,2)],'k' ,'HandleVisibility','off');
            end
        end
    end
end
...
for c=1:n_cs
    for t=1:n_tp
        if x(c,t) == 1
            if first
                plot([cs_positions(c,1) tp_positions(t,1)],[cs_positions(c,2) tp_positions(t,2)], 'k-.', 'DisplayName', 'ACC Link','Linewidth',1);
                first=0;
            else
                plot([cs_positions(c,1) tp_positions(t,1)],[cs_positions(c,2) tp_positions(t,2)], 'k-.',  'HandleVisibility','off','Linewidth',1);
            end
        end
    end
end
