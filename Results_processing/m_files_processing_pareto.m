...
for d = 1:numel(dir_name_list)
    ...
    for sol=1:numel(solutions)
        ...
        avg_rel(d) = avg_rel(d) + sum(y_rel);
        
        %avg_cost(d) = avg_cost(d) + sum(y_iab)*iab_price + sum(y_rel)*iab_price;
        
%         temp_len = 0;
%         for t=1:n_tp
%             for c=1:n_cs
%                     if x(c,t)==1
%                         temp_len = temp_len + cs_tp_distance_matrix(c,t);
%                     end
%             end
%         end
...
%         avg_linlen(d)     = avg_linlen(d) + temp_len;
                        ...
                        for r=1:n_cs
                            ...
                            if s(t,c,r)==1
                                if r == n_cs
                                    ...
                                    temp_len = temp_len + cs_tp_distance_matrix(r,t);
                                end
                            end
                        end
                    end
    ...
    avg_rel(d) = avg_rel(d)/numel(solutions);
    
    avg_cost(d) = avg_rel(d).*rel_price + avg_iab(d).*iab_price;
